# demo_yolo_obb.py - YOLOv8 oriented bounding box demo
...
class YOLOOBBDemo(Application, AIBase):
    name = "YOLO OBB"
    model_path = config.KMODEL_DIR + "yolov8n-obb.kmodel"
    model_size = [320, 320]
    rgb_size   = [1280, 720]
    ...
    def stop(self):
        ...
        if self._video_bound:
            try:
                ...
                print("YOLOOBBDemo: video layer disable failed:", e)
            self._video_bound = False
        if sensor is not None:
            try:
                sensor.stop()
            except Exception as e:
                print("YOLOOBBDemo: sensor stop failed:", e)
    ...
    def postprocess(self, results):
        ...
        obb_res = aidemo.yolo_obb_postprocess(
            new_result.copy(),
    ...
    def draw_result(self, img, dets):
        ...
        if dets and len(dets) >= 3 and dets[0]:
            for i in range(len(dets[0])):
                x1, y1, x2, y2, x3, y3, x4, y4 = map(lambda v: int(round(v, 0)), dets[0][i])
                cls_id = int(dets[1][i])
                color = self.colors[cls_id % len(self.colors)]
                visual.draw_polygon(
                    img, [(x1, y1), (x2, y2),
                          (x3, y3), (x4, y4)],
                    label=self.LABELS[cls_id], score=dets[2][i],
                    color=color)
                count += 1
        return count
