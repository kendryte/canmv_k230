# demo_openmv_find_circles.py - Independent OpenMV circle detection application
...
class OpenMVFindCirclesApp(Application):
    name = "OpenMV 圆形"
    model_path = ""
    required_files = []
    is_audio_only = False
    ...
    def run_once(self):
        ...
        circles = self.frame.find_circles(
            threshold=2000, x_margin=10, y_margin=10, r_margin=10,
            r_min=2, r_max=100, r_step=2)
        count = 0
        for circle in circles:
            self.frame.draw_circle(circle.x(), circle.y(), circle.r(),
                                   color=(0, 255, 0))
            count += 1
        self.frame.draw_string_advanced(
            8, 8, 16, "OpenMV Circles", color=(0, 255, 0))
    ...
    def stop(self):
        ...
        try:
            ...
            print("OpenMVFindCirclesApp: layer cleanup failed:", e)
        if sensor is not None:
            try:
                sensor.stop()
            except Exception as e:
                print("OpenMVFindCirclesApp: sensor stop failed:", e)
