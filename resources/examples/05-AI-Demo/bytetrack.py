...
def main():
    ...
    try:
        ...
        # 初始化多目标跟踪器
        tracker=mot.ByteTrack(track_thresh=track_thresh,high_thresh=high_thresh,match_thresh=match_thresh,track_buffer=track_buffer,frame_rate=frame_rate,min_hits=min_hits,class_aware=True,bbox_format="xywh")
        ...
        try:
            ...
            print("ByteTrack cleanup tracker failed:", error)
        tracker=None
        try:
            if person_det is not None:
                person_det.deinit()
        except BaseException as error:
            print("ByteTrack cleanup detector failed:", error)
        person_det=None
        try:
            if pl is not None:
                pl.destroy()
        except BaseException as error:
            print("ByteTrack cleanup pipeline failed:", error)
        pl=None
        gc.collect()
