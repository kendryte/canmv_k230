...
def main():
    ...
    try:
        ...
        # 初始化多目标跟踪器
        tracker=mot.BoTSort(feature_dim=feature_dim,reid_enabled=True,track_high_thresh=track_high_thresh,track_low_thresh=track_low_thresh,new_track_thresh=new_track_thresh,track_buffer=track_buffer,match_thresh=match_thresh,proximity_thresh=proximity_thresh,appearance_thresh=appearance_thresh,appearance_weight=appearance_weight,frame_rate=frame_rate,min_hits=min_hits,class_aware=True,bbox_format="xywh")
        ...
        try:
            ...
            print("BoTSort cleanup tracker failed:", error)
        tracker=None
        try:
            if feature_app is not None:
                feature_app.deinit()
        except BaseException as error:
            print("BoTSort cleanup feature model failed:", error)
        feature_app=None
        try:
            if person_det is not None:
                person_det.deinit()
        except BaseException as error:
            print("BoTSort cleanup detector failed:", error)
        person_det=None
        try:
            if pl is not None:
                pl.destroy()
        except BaseException as error:
            print("BoTSort cleanup pipeline failed:", error)
        pl=None
        gc.collect()
